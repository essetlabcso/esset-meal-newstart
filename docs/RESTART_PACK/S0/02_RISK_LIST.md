...
| R7 | **Projection drift: ghost rows duplicate nodes or unstable ordering** | Option C contract breaks; matrix reports become inconsistent | Keep projection reads derived on demand with deterministic tie-breakers; enforce projection unit + SQL proofs | `tests/unit/projectionContract.test.mjs`, `supabase/tests/s0_toc_projection_contract.sql`, `supabase/migrations/20260218113000_phase_e_projection_matrix_read_model.sql` |
