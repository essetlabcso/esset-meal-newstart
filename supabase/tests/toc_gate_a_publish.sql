-- Gate A publish proof: blocker + pass + copy-on-write draft creation
...
begin
  ...
  values
    (v_org, v_draft, 'GOAL', 'Goal', v_user),
    (v_org, v_draft, 'OUTCOME', 'Orphan Outcome', v_user)
  ...
  exception when others then
    if position('Gate A failed' in sqlerrm) = 0 then
      reset role;
      raise;
    end if;
  end;

  insert into public.toc_edges (tenant_id, toc_version_id, source_node_id, target_node_id, edge_type, created_by)
  values (v_org, v_draft, v_goal, v_outcome, 'CONTRIBUTES_TO', v_user);
